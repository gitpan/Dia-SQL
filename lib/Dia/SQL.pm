...
# $Id: SQL.pm 448 2009-02-20 11:47:06Z aff $
...
Dia::SQL is the parser that interprets the .dia file(s) into an
internal datastructure.

Dia::SQL::Output (or one of its sub classes) can take the
datastructure and generate SQL statements it represents.

=head1 DATABASE SUPPORT NOTE

As of VERSION 0.01 I<the only supported database is DB2>.  The aim is
to suppport the same set of databases as I<tedia2sql>.
...
sub get_output_instance {
  ...
  if ($self->{db} eq q{db2}) {
	return Dia::SQL::Output::DB2->new(%param);
  }
